...
// ──────────────────────────────────────────────
let notifications = JSON.parse(localStorage.getItem('hm_notifications') || '[]');

function saveNotifications() {
    localStorage.setItem('hm_notifications', JSON.stringify(notifications.slice(0, 50)));
}
...
async function startFirmwareUpdate() {
    ...
    if (!btDevice?.gatt?.connected) {
        toast('⚠️ Please connect your HeightMate device first via Bluetooth!');
        return;
    }
